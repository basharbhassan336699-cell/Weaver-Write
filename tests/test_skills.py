...
ok("  ⟵ بطلبٍ من المستخدم فقط (لا في كلِّ PDF)",
   "بطلبٍ من المستخدم فقط" in _cp and "لم يطلب الصفحة" in _cp)
ok("  ⟵ والنموذجُ يفهم القصد، لا كلمةٌ بعينها", "لا من كلمةٍ بعينها" in _cp)
_cpd = front(BODY["cite-pages"], "description")
ok("  ⟵ والوصفُ (يراه دائماً) يقولها: Only when the user asks",
   _cpd.startswith("Only when the user asks"))
ok("  ⟵ وفيه العرضُ بسطرٍ حين يُقتبَس من PDF ولم يُطلب",
   "offer them in one line" in _cpd)
ok("  ⟵ والطلبُ المعاكسُ للاختبار موجود",
   "cite-pages" in getattr(wc, "_SKILL_NEGATIVE", {}))
